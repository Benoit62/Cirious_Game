...
if(!empty($_POST) && ($_POST['valider']) == 'inscription'){
    ...
    if(!empty($pseudo) && !empty($mail) && !empty($mdp) && !empty($confirm_mdp) && taille_chaine($pseudo, 30) && taille_chaine($mail, 100) && taille_chaine($mdp, 256)){
        
        if (filter_var($mail, FILTER_VALIDATE_EMAIL)) {

            $verifmail = $bdd->prepare('SELECT * FROM comptes WHERE mail = ?');
