...
<<<<<<< Updated upstream
    ...
    this.cameras.main.setBounds(
      farm.x - farm.width / 2 - 366,
      farm.y - farm.height / 2 - 60,
      farm.width + 366,
      farm.height + 60
    );
    this.physics.world.setBounds(
      farm.x - farm.width / 2,
      farm.y - farm.height / 2,
      farm.width,
      farm.height
    );
      ...
      if (bat.dataType == "bat") {
        if (bat.level > 0 && bat.tag != "build") {
          ...
          if (bat.type == "field") {
            if (!bat.plant) {
              let arrayField = [];
              arrayField["ground"] = this.physics.add.image(
                bat.x,
                bat.y,
                bat.tag + this.gameScene,
                bat.level - 1
              );
              arrayField["weeds"] = this.add.image(
                bat.x,
                bat.y,
                "weeds",
                bat.weeds
              ...
              arrayField["plant"] = this.add.image(
                bat.x,
                bat.y,
                bat.tag,
                bat.grow
...
>>>>>>> Stashed changes
          ...
          this.images.push(
            this.physics.add
              .image(bat.x, bat.y, "build" + this.gameScene)
              .setScale(bat.scale)
          );
        }
        if (
          bat.type == "field" &&
          bat.level > 0 &&
          bat.tag != "build" + this.gameScene
        ) {
          this.physics.add.overlap(
            this.player,
            this.images[j]["ground"],
            this.overlapBat,
            null,
            this
          );
    ...
    var dataRiv = [
      -1500, -17, -850, -17, -827, -55, -800, -55, -800, 125, -840, 125, -872,
      155, -1500, 155, -1500, 124, -735, 124, -705, 90, -595, 90, -547, 30,
      -135, 30, -90, 0, -35, 0, -5, -35, 420, -35, 485, 25, 520, 25, 560, 70,
      575, 70, 575, 60, 635, 60, 800, 60, 825, 90, 1500, 90, 1500, -120, 910,
      -120, 910, -57, 823, -57, 803, -25, 690, -25, 690, -59, 640, -59, 640, 60,
      575, 60, 575, -59, 505, -59, 480, -89, 52, -89, 52, -320, 83, -375, 83,
      -385, 83, -500, 100, -500, 120, -520, 120, -1500, -56, -1500, -56, -500,
      83, -500, 83, -385, -56, -385, -85, -372, -85, -215, -115, -215, -115,
      -150, -140, -150, -160, -120, -460, -120, -475, -90, -585, -90, -600, -60,
      -735, -60, -735, 124, -1500, 124, -1500, -17,
    ];
    // The boundary
    ...
    this.input.keyboard.on(
      ...
      function () {
        this.registry.set(
          "animalCare" + this.gameScene,
          this.registry.get("animalCare" + this.gameScene) < 90
            ? this.registry.get("animalCare" + this.gameScene) + 10
            : this.registry.get("animalCare" + this.gameScene)
        );
      },
      this
    );
    ...
    this.input.keyboard.on(
      ...
      function () {
        this.registry.set(
          "ecology" + this.gameScene,
          this.registry.get("ecology" + this.gameScene) < 90
            ? this.registry.get("ecology" + this.gameScene) + 10
            : this.registry.get("ecology" + this.gameScene)
        );
      },
      this
    );
    ...
    this.input.keyboard.on(
      ...
      function () {
        this.registry.set(
          "hunger" + this.gameScene,
          this.registry.get("hunger" + this.gameScene) < 90
            ? this.registry.get("hunger" + this.gameScene) + 10
            : this.registry.get("hunger" + this.gameScene)
        );
      },
      this
    );
      ...
      for (let i in this.data.values) {
        ...
        if (bat.level > 0 && bat.tag != "build" && bat.type == "struct") {
          if (typeof bat.ref.passif[bat.level] == "number") {
            ...
            getByType(bat.ref.product).forEach((value) =>
              value.unlock ? (moneyPerSec += value.passif) : (moneyPerSec += 0)
            );
          }
        }
      ...
      if (bat.level < bat.ref.lvlMax && bat.level != 0) {
        if (this.money() >= bat.ref.upgrade[bat.level + 1]) {
          if (bat.type != "animal") {
            ...
            this.registry.set(
              "money",
              this.registry.get("money") - bat.ref.upgrade[bat.level]
            );
          ...
          if (bat.type == "animal") {
            ...
            let textMoney = this.add
              .text(bat.x, bat.y, "-" + bat.ref.upgrade[bat.level], {
                lineSpacing: 10,
                fontSize: 40,
                color: "#ffffff ",
              })
              .setOrigin(0.5, 0.5);
            let moneyButton = this.add
              .image(textMoney.x + textMoney.width / 1.5, textMoney.y, "dollar")
              .setScale(0.08)
              .setOrigin(0, 0.5);

            let textAnimal = this.add
              .text(
                bat.x,
                textMoney.y + textMoney.height * 0.8,
                "+" + (10 * bat.level).toString(),
                { lineSpacing: 10, fontSize: 40, color: "#ffffff " }
              )
              .setOrigin(0.5, 0.5);
            let animalButton = this.add
              .image(
                textAnimal.x + textAnimal.width / 1.5,
                textAnimal.y,
                "animal-care"
              )
              .setScale(0.08)
    ...
    if (bat.level == 0 && bat.tag == "build") {
      if (this.money() >= ref.buildCost) {
        ...
        if (bat.type != "field") {
          ...
          this.images[bat.key - 1] = this.physics.add.image(
            bat.x,
            bat.y,
            bat.tag + this.gameScene,
            bat.level - 1
          ...
          if (bat.type == "animal") {
            ...
            let textMoney = this.add
              .text(bat.x, bat.y, "-" + ref.buildCost, {
                lineSpacing: 10,
                fontSize: 40,
                color: "#ffffff ",
              })
            ...
            let textAnimal = this.add
              .text(bat.x, textMoney.y + textMoney.height * 0.8, "-40", {
                lineSpacing: 10,
                fontSize: 40,
                color: "#ffffff ",
              })
            ...
            let textEcology = this.add
              .text(bat.x, textAnimal.y + textAnimal.height * 0.8, "-40", {
                lineSpacing: 10,
                fontSize: 40,
                color: "#ffffff ",
              })
              .setOrigin(0.5, 0.5);
            let ecologyButton = this.add
              .image(
                textEcology.x + textEcology.width / 1.5,
                textEcology.y,
                "ecology-care"
              )
              .setScale(0.08)
          ...
          this.images[bat.key - 1]["ground"] = this.physics.add.image(
            bat.x,
            bat.y,
            ref.tag + this.gameScene,
            bat.level - 1
          );
          this.images[bat.key - 1]["weeds"] = this.add.image(
            bat.x,
            bat.y,
            "weeds",
            bat.weeds
...
<<<<<<< Updated upstream
  ...
  plant(bat, seed) {
    ...
    if (
      ...
      !bat.plant &&
      bat.level == 1 &&
      (bat.tag == "labor" || bat.tag == "water")
    ) {
      if (this.money() >= seed.costPlant) {
        if (bat.tag == seed.ground) {
          this.registry.set(
            "money",
            this.registry.get("money") - seed.costPlant
          );
          bat.plant = true;
          bat.tag = seed.tag;
          bat.seed = seed;
          console.log("Planted !", bat);
          this.images[bat.key - 1]["plant"] = this.add.image(
            bat.x,
            bat.y,
            seed.tag,
            bat.grow
          ...
          if (bat.rotate)
            this.images[bat.key - 1]["plant"].rotation = 3.141592 / 2;

          let textMoney = this.add
            .text(bat.x, bat.y, "-" + seed.costPlant, {
              lineSpacing: 10,
              fontSize: 40,
              color: "#ffffff ",
            })
...
>>>>>>> Stashed changes
          ...
          this.menuScene.errorText(
            "Vous ne pouvez pas planter cette culture sur ce type de champs"
          );
        }
      } else {
        console.log("Not enought money");
      ...
      if (bat.level == 1 && bat.type == "field") {
        if (Phaser.Math.Between(1, 5) <= 2) {
          ...
          let textHealth = this.add
            ...
            .setOrigin(0.5, 0.5);
          let healthButton = this.add
            .image(
              textHealth.x + textHealth.width / 1.5,
              textHealth.y,
              "health"
            )
            .setScale(0.08)
...
>>>>>>> Stashed changes
  ...
  clean(bat, lutte) {
    ...
    if (bat.type == "field" && bat.level == 1 && bat.weeds > 0) {
      ...
      let textHealth = this.add
        .text(
          bat.x,
          textEcology.y + textEcology.height * 0.8,
          "+" + lutte.health,
          { lineSpacing: 10, fontSize: 40, color: "#ffffff " }
        )
        .setOrigin(0.5, 0.5);
      let healthButton = this.add
        .image(textHealth.x + textHealth.width / 1.5, textHealth.y, "health")
        .setScale(0.08)
        .setOrigin(0, 0.5);
  ...
  recolte(bat) {
    ...
    if (
      ...
      bat.level == 1 &&
      (bat.tag != "labor" || bat.tag != "water") &&
      (bat.grow == bat.seed.maxGrow || bat.dead)
    ) {
      let percent = bat.fertility / 100;
      let percent2 = (bat.maxWeeds - bat.weeds) / 10;
      console.log(percent, percent2);
      let moneyWin =
        Math.round(bat.seed.money * percent * percent2 * 100) / 100;
      ...
      if (!bat.dead) {
        ...
        let moneyButton = this.add
          ...
          .setScale(0.08)
          .setOrigin(0, 0.5);
        let textHunger = this.add
          .text(bat.x, textMoney.y + textMoney.height * 0.8, hungerWin, {
            lineSpacing: 10,
            fontSize: 40,
            color: "#ffffff ",
          })
          .setOrigin(0.5, 0.5);
        let hungerButton = this.add
          .image(
            textHunger.x + textHunger.width / 1.5,
            textHunger.y,
            "hunger-care"
          )
          .setScale(0.08)
        ...
        let textHealth = this.add
          .text(
            bat.x,
            textHunger.y + textHunger.height * 0.8,
            "-" + looseFertility,
            { lineSpacing: 10, fontSize: 40, color: "#ffffff " }
          )
          .setOrigin(0.5, 0.5);
  ...
  fertility(bat, engrais) {
    ...
    if (
      ...
      let textFertility = this.add
        ...
        .setOrigin(0.5, 0.5);
      let fertilityButton = this.add
        .image(
          textFertility.x + textFertility.width / 1.5,
          textFertility.y,
          "fertility"
        )
        .setScale(0.08)
  ...
  destroyBat(bat, destroyRef) {
    ...
    if (
      (bat.level > 0 && bat.tag != "build" && bat.type == "animal") ||
      bat.type == "struct" ||
      bat.type == "field"
    ) {
      if (this.money() >= destroyRef.cost) {
        if (bat.type == "animal") {
          ...
          if (!bat.dead) {
            ...
            let textMoney = this.add
              .text(bat.x, bat.y, "-" + destroyRef.cost, {
                lineSpacing: 10,
                fontSize: 40,
                color: "#ffffff ",
              })
            ...
            let textAnimal = this.add
              .text(bat.x, textMoney.y + textMoney.height * 0.8, +20, {
                lineSpacing: 10,
                fontSize: 40,
                color: "#ffffff ",
              })
            ...
            let textEcology = this.add
              .text(bat.x, textAnimal.y + textAnimal.height * 0.8, "+30", {
                lineSpacing: 10,
                fontSize: 40,
                color: "#ffffff",
              })
        ...
        this.registry.set(
          "money",
          this.registry.get("money") - destroyRef.cost
        );
        if (bat.type == "field") {
          ...
          let textEcology = this.add
            .text(bat.x, textMoney.y + textMoney.height * 0.8, "+10", {
              lineSpacing: 10,
              fontSize: 40,
              color: "#ffffff",
            })
...
>>>>>>> Stashed changes
          ...
          if (Phaser.Math.Between(1, 5) <= 4) {
            ...
            if (bat.feed <= 0) {
              ...
              setTimeout(() => {
                bat.dead = true;
                console.log("Dead animal !", bat);
                this.images[bat.key - 1].setFrame(
                  bat.level - 1 + bat.ref.lvlMax
                );
      ...
      if (bat.level > 0 && bat.type == "animal" && !bat.dead && bat.qt < 100) {
        if (bat.feed >= 35) {
          if (Phaser.Math.Between(1, 10) <= 7) {
            ...
            if (bat.qt < 75)
              this.images[bat.key - 1].setFrame(
                bat.level - 1 + 4 * bat.ref.lvlMax
              );
            if (bat.qt < 50)
              this.images[bat.key - 1].setFrame(
                bat.level - 1 + 2 * bat.ref.lvlMax
              );
            if (bat.qt < 25)
              this.images[bat.key - 1].setFrame(
                bat.level - 1 + 3 * bat.ref.lvlMax
              );
          }
        }
        if (bat.feed < 35 && bat.feed >= 30) {
  ...
  sell(bat, sell) {
    ...
    if (bat.type == "animal" && !bat.dead && bat.level > 0 && bat.qt > 40) {
      ...
      if (bat.qt <= 100) this.images[bat.key - 1].setFrame(bat.level - 1);
      if (bat.qt < 75)
        this.images[bat.key - 1].setFrame(bat.level - 1 + 4 * bat.ref.lvlMax);
      if (bat.qt < 50)
        this.images[bat.key - 1].setFrame(bat.level - 1 + 2 * bat.ref.lvlMax);
      if (bat.qt < 25)
        this.images[bat.key - 1].setFrame(bat.level - 1 + 3 * bat.ref.lvlMax);
      ...
      let texthunger = this.add
        .text(
          bat.x,
          textAnimal.y + textAnimal.height * 0.8,
          sell.hunger.toString(),
          { lineSpacing: 10, fontSize: 40, color: "#ffffff " }
        )
        .setOrigin(0.5, 0.5);
      let hungerButton = this.add
